...
> **다음 세션 재개 지점**
>
> **병행 학습:** 모던 JS(ES2015~2021) 기본 — 배열 메서드(map/reduce/filter), lookup 객체, `??`/`?.`/`??=`, 구조분해. 프로젝트 막힘의 8할이 React가 아니라 여기였음. 프로젝트 조금씩 하면서 같이.

### 1순위 — 데이터를 axios로 가져오기 ← 다음 세션 시작점
- [ ] 먼저 **JSON 데이터 파일** 만들어두고 (mock), axios로 fetch하는 형태로 전환 (지금은 컴포넌트에 하드코딩 — schedule/expenseData/pieData 등)
...
- [ ] cd3·cd5·cd7 차트/그리드 고정 px(`200`/`260`/`240`) → `flex-1 min-h-0` + `height:100%` 레시피로 정리 (cd6에서 검증됨)
- [ ] `id` 중복/콘텐츠 div id 정리
