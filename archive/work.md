...
### 1순위 — 일정 카드(cd4) 채우기 ← 다음 세션 시작점
- [ ] **일정 카드** 리스트 구현. **구현 방식 결정 필요** (다음 세션에서 정함):
  - **MUI** (`List`/`ListItem`) 사용 vs **직접** `ul`/`map` + Tailwind
  - 앞서(2026-05-31) "MUI는 디자인 톤 섞임"이라 직접 map으로 기운 적 있음 → 재검토
  - 일정 **데이터 배열 구조부터** 잡기 (`divide-y`로 행 구분선)
- [ ] cd4_tit 아이콘 원형 래퍼 비어있음(다른 카드처럼 틴트 원 배경 적용)

### 2순위 — Dashboard 마무리 정리
- [ ] 카드 껍데기 반복 → `<DashboardCard title icon kind>` **컴포넌트로 추출** (지금 손으로 다 그려둔 상태 → 추출 타이밍)
- [ ] `id` 중복/콘텐츠 div id 정리

### 3순위 — 데이터를 axios로 가져오기
- [ ] 먼저 **JSON 데이터 파일** 만들어두고 (mock), axios로 fetch하는 형태로 전환 (지금은 컴포넌트에 하드코딩 — expenseData/pieData 빈 배열로 두고 기본값 fallback 쓰는 중)
...
| 회원가입 | ⬜ 미작업 |
| 비밀번호 찾기 | ⬜ 미작업 |
| 대시보드 | 🟡 진행중 (카드 7개 중 6개 내용 완료 — 예산/지출/전자결재/지출현황/Plan/가족구성원. **일정 카드(cd4)만 남음**) |
| 일정 | ⬜ 미작업 |
| 가계부 | ⬜ 미작업 |
| 가계부 보고서 | ⬜ 미작업 |
| 전자결재 | ⬜ 미작업 |
