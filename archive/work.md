...
> **다음 세션 재개 지점**
>
> **프로젝트 재개 지점:** 탭 시스템에서 **콘텐츠 전환**(`activeTab` → 실제 화면 렌더)이 다음. 지금은 `<Outlet/>`이라 탭 바꿔도 내용은 대시보드 그대로. activeTab으로 분기 렌더(라우터 연결은 그 뒤 별도 단계). 그 후 **react-실전 01강 = 가계부 입력 폼**(제어 컴포넌트)으로 가계부 화면 착수.
...
- [ ] **(다음) 탭 콘텐츠 전환** — `activeTab` → 해당 화면 렌더(현재 `<Outlet/>`이라 미연결).
- [ ] 탭 닫기(X 버튼, `filter`로 제거), 탭 ↔ 라우트 연결(`navigate`+`useLocation`), 메뉴 클릭→탭 추가를 깊은 곳에서도 쓰면 Context로 승급.
